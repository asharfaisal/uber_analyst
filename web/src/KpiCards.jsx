export default function KpiCards({ summary, highlightKey }) {
  ...
  // (full-width, emphasized) instead of the whole grid -- a "total
  // customers" question shouldn't also dump revenue, ratings, etc.
  if (highlightKey) {
  ...
  return (
    <div className="kpi-grid">
      {cards
        .filter((c) => c.key !== 'unique_customers') // keep default overview grid to the original 6
        .map((c) => (
